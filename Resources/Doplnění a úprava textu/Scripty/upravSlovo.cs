...
public class upravSlovo : MonoBehaviour {
	...
	void Start () {
		...
		for(int i = 0; i < seznamObrazku.Length; i++){
			poradi.Add (i);
			iF.Add (seznamObrazku[i].GetComponent<upravSlovoObr>().iF);
		}
		inicializace();
	}
	...
	public void iField(string str){
				...
				if(iF[poradi[cisloPole]].text == seznamObrazku[poradi[cisloPole]].GetComponent<upravSlovoObr>().SpravneSlovo){
	...
	public void iFieldEnd(string str){
		if(iF[poradi[cisloPole]].text != seznamObrazku[poradi[cisloPole]].GetComponent<upravSlovoObr>().SpravneSlovo  && iF[poradi[cisloPole]].text.Length >0 ){
	...
	public void inicializace(){
		...
		for (int i = 0; i <poradi.Count; i++){
			seznamObrazku[poradi[i]].GetComponent<upravSlovoObr>().poradi = i;
		}
	...
	public void nastavSpatnaSlova(){
		for(int i = 0; i < seznamObrazku.Length; i++){
			...
			iF[i].text = seznamObrazku[i].GetComponent<upravSlovoObr>().SpatneSlovo;
